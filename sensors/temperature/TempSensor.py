...
class TempSensor(Sensor):
    ...
    def getHumidity(self):
        if self._sensorType == const._sens_dht22 and self._sensor_function_humidity == "1":
            humidity, temperature = Adafruit_DHT.read_retry(Adafruit_DHT.DHT22, self._sensor_pin)
            return humidity
        else:
            return 0
    ...
    def getPressure(self):
        if self._sensorType == const._sens_bmp085 and self._sensor_function_pressure == "1":
            self.bmp = BMP085.BMP085()
    ...
    def getAltitude(self):        
        if self._sensorType == const._sens_bmp085 and self._sensor_function_altitude == "1":
            self.bmp = BMP085.BMP085()
